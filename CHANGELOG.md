...
### CHANGED
- Released a patch version containing the HTTP 409 (`Conflict`) response mapping.

## [1.3.0] - 2026-09-07

### ADDED
- Mapped `ConflictError` to HTTP 409 (`Conflict`) in `CustomControllerBase` for both `Result` and `Result<T>` overloads.

### CHANGED
- **Bumped** `IATec.Shared.Domain` from `2.0.0` to `2.1.0`.
